...
ShaderData {
    struct ProvinceData {
        uint population;                // Population in thousands (discrete)
        float foodProductionModifier;   // Production multiplier
        uint wealth;                    // Wealth in units (discrete)
        uint foodStorage;               // Food units stored (discrete)
    };
    
    struct AggregateData {
        uint totalPopulation;           // Changed to uint
        uint totalWealth;               // Changed to uint
        float avgGrowth;
        uint growing;
        uint stable;
        uint declining;
    ...
    InputData {
        ...
        float wealthPerPop;
        uint maxFoodStorage;            // Changed to uint
        uint minPopulation;             // Changed to uint
    };
