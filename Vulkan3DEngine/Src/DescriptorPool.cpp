...
DescriptorPool::DescriptorPool(uint32_t objectCount, Renderer* renderer): m_renderer(renderer)
{
    ...
    poolSizes[0].descriptorCount = (1 + objectCount) * static_cast<uint32_t>(maxFramesinFlight);//Always number of frames in flight
    poolSizes[1].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    poolSizes[1].descriptorCount = static_cast<uint32_t>(maxFramesinFlight) * objectCount;//number of frames in flight times number of textures

    VkDescriptorPoolCreateInfo poolInfo{};
    poolInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO;
    poolInfo.poolSizeCount = static_cast<uint32_t>(poolSizes.size());
    poolInfo.pPoolSizes = poolSizes.data();
    poolInfo.maxSets = (1 + objectCount * 2) * static_cast<uint32_t>(maxFramesinFlight);//1 for global descriptorset, 1 for mesh and 1 for texture descriptor set
